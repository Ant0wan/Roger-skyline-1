...
def configure_vm(dinfo):
    ...
    system("vboxmanage modifyvm " + dinfo['VM_name'] \
            + " --cpus " + dinfo['cpu'] \
            + " --memory " + dinfo['ram'])
    create_disk(dinfo['VM_name'], dinfo['disk_size'], dinfo['VDI_path'], dinfo['iso_path'])
    boot_order(dinfo['VM_name'])
#BUG#    system("VBoxManage modifyvm " + VM_name + " --nic2 bridged  --bridgeadapter1 " + NAT2)

def create_disk(VM_name, disk_size, disk_path, iso_path):
    storage = ('IDE', 'SATA')
    ...
    system("VBoxManage storageattach " + VM_name \
            ...
            + " --medium " + disk_name + disk_path + disk_file_format)
